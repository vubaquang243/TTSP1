...
<div class="box_common_conten">
  <html:form action="loadDuyetLTTTinh.do" method="post" >
    ...
    <tbody>
      <tr>
        ...
        <td background="<%=request.getContextPath()%>/styles/images/T2.jpg" width="75%">
          <span class=title2> LTT tỉnh cần duyệt </span>
        </td>
           ...
           <td colspan="2">
            ...
            <legend><font color="Blue">Danh s&#225;ch ch&#7901; duy&#7879;t</font></legend>
              ...
              <table  class="data-grid" id="data-grid" 
                     ...
                     <td align="center" width="19%" class="ui-state-default ui-th-column">KH chuy&#7875;n</td>
                     <td align="center" width="19%" class="ui-state-default ui-th-column">T&#224;i kho&#7843;n - KH nh&#7853;n</td>
                     <td align="center" width="19%" class="ui-state-default ui-th-column">NH nh&#7853;n </td>                     
                   ...
                   <logic:notEmpty name="colLTT">
                     ...
                     <td align="left"  onmouseover="style.fontWeight='bold'" onmouseout="style.fontWeight='normal'">
                       ...
                       <td align="left" >
                       <div title="<bean:write name="items"  property="ten_nhkb_nhan"/>" style="text-overflow:ellipsis;white-space:nowrap;  width:180px; overflow:hidden; font-size:13px">
                       ...
                       <td align="right"  style="font-size:12px">
                        <input type="text" name="ly_do_tinh" id="ly_do_tinh_<bean:write name="index"/>" onkeydown="arrowUpDownTinh(event,'<bean:write name="index"/>')" style="width:98%;background-color:white" class="fieldTextLeft"/>
...
jQuery('#so_ltt').keyup(function () {
 ...
 jQuery('#tien_min').keyup(function () {
            var matches = /[^0-9]/g;
            jQuery('#so_ltt').val(this.value.replace(matches, ''),true);
         });
   ...
   jQuery('#id_ldo').val('');
     if (type == 'find') {
       var idxKB = jQuery('#nhkb_huyen option:selected').index();
        f.action = 'loadDuyetLTTTinh.do?idxKB='+idxKB      
      }
      if (type == 'update') {
        ...
        if (check_item.length < 1){
          alert("Cần chọn LTT cần duyệt");
            ...
            val[i] = jQuery(this).val();
              ...
              var ly_do= jQuery('#ly_do_tinh_'+val1.substr(val1.lastIndexOf('yctt@seatechit')+14)).val();
                 if(ly_do.trim()==null || ''==ly_do.trim()){
                    jQuery('#id_ldo').val('#ly_do_tinh_'+val1.substr(val1.lastIndexOf('yctt@seatechit')+14));
                    return false;                   
                 }
           });
           ...
           if(ldo_empty!=null && ''!=ldo_empty){
            ...
            f.action = 'updateDuyetLTTTinh.do?type=DUYET';      
      }
      if (type == 'ko_duyet') {
        var check_item = jQuery('input[type=checkbox]:checked');
        if (check_item.length < 1){
          alert("Cần chọn LTT không duyệt");
           ...
           if(ldo_empty!=null && ''!=ldo_empty){
            ...
            f.action = 'updateDuyetLTTTinh.do?type=KO_DUYET';      
      }
     if (type == 'close') {
...
function goPage(page) {
      ...
      f.action = 'loadDuyetLTTTinh.do?pageNumber='+page;
      f.submit();
  }
...
function makeGetRequestView(id, type) {
      var urlRequest = null;
          urlRequest = "listLttAdd.do?action=VIEW_LTTDi_DUYETLO&so_ltt_details=" + id + "&typeLO=TINH";
...
function arrowUpDownTinh(e,idx_ldo) {
...
//        alert(parseInt(idx_ldo)+1);
        ...
        jQuery('#ly_do_tw_'+idx_ldo).css('background-color', 'white');
            jQuery('#ly_do_tinh_'+nxtIdx).focus();
            break;
        case arrow.esc:
           jQuery('#ly_do_tinh_'+idx_ldo).val('');
            break;
    }
