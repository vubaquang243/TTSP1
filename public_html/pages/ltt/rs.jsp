...
//      });
      ...
      jQuery("#btn_timKiem").click(function () {
          if (jQuery("#tu_ngay").val() == null || jQuery("#tu_ngay").val() == ''){
            alert('Hãy nhập ngày thanh toán để thực hiện tìm kiếm!');
          ...
          frmTraCuuLTT.attr( {
              action : 'resendLTTListAction.do' + target
          });
          frmTraCuuLTT.submit();
      });
      //**************************BUTTON Thoat CLICK********************************************
  ...
  jQuery("#frmTraCuuLTT").target='';
      jQuery("#frmTraCuuLTT").attr( {
          action : 'resendLTTAction.do' + target
      });
  ...
  function formAction(type) {       
      ...
      if(lstId!=""){
        var loai_lenh = jQuery("#loai_lenh option:selected").val();
        if(loai_lenh == '01'){
          f.action = "resendLTTAction.do?type="+type+"&lstId="+lstId+"&loai_tt=LTT_DI";
        }else{
          f.action = "resendLTTAction.do?type="+type+"&lstId="+lstId+"&loai_tt=LTT_DEN";
        }
      }else{
...
//      jQuery("#ma_field_id_lov").val("");
//      jQuery("#ten_field_id_lov").val("");
//      jQuery("#id_field_id_lov").val("");
//      jQuery("#ma_cha_field_id_lov").val("");
