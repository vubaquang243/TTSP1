package com.seatech.ttsp.ttthanhtoan;


public class TTThanhToanVO {
  private String id;  // msg_id
  private String msg_refid;
  private String nhkb_chuyen;
  private String nhkb_nhan;
  private String ngay_tao;
  private String kbac_dv;
  private String loai_tk;
  private String so_tk;
  private String cn_nh;
  private String ten_cn_nh;
  private String han_muc_no;
  private String du_dau_ngay;
  private String ps_thu;
  private String ps_chi;
  private String ma_nh;
  private String ten_nh;
  
  private String lan_tcuu;
  private String ngay_tcuu;
  private String ma_kb;
  private String ten_kb;
  private String kb_id;
  private String chenh_lech;
  
  
  private Long nguoi_tao;
  private String so_lan;
  private String trang_thai;
  private String ma_kq;
  
  private String ma;
  private String ten;
  private String ma_cha;
  private String id_cha;
  private String cap;
  private String nhkb_nhan_cn;
  
  private String msg_id;
  private String nh_id;
  
  private String loai_tien;

    public void setId(String id) {
        this.id = id;
    }

    public String getId() {
        return id;
    }

    public void setMsg_refid(String msg_refid) {
        this.msg_refid = msg_refid;
    }

    public String getMsg_refid() {
        return msg_refid;
    }

    public void setNhkb_chuyen(String nhkb_chuyen) {
        this.nhkb_chuyen = nhkb_chuyen;
    }

    public String getNhkb_chuyen() {
        return nhkb_chuyen;
    }

    public void setNhkb_nhan(String nhkb_nhan) {
        this.nhkb_nhan = nhkb_nhan;
    }

    public String getNhkb_nhan() {
        return nhkb_nhan;
    }

    public void setNgay_tao(String ngay_tao) {
        this.ngay_tao = ngay_tao;
    }

    public String getNgay_tao() {
        return ngay_tao;
    }

    public void setKbac_dv(String kbac_dv) {
        this.kbac_dv = kbac_dv;
    }

    public String getKbac_dv() {
        return kbac_dv;
    }

    public void setLoai_tk(String loai_tk) {
        this.loai_tk = loai_tk;
    }

    public String getLoai_tk() {
        return loai_tk;
    }

    public void setSo_tk(String so_tk) {
        this.so_tk = so_tk;
    }

    public String getSo_tk() {
        return so_tk;
    }

    public void setCn_nh(String cn_nh) {
        this.cn_nh = cn_nh;
    }

    public String getCn_nh() {
        return cn_nh;
    }

    public void setTen_cn_nh(String ten_cn_nh) {
        this.ten_cn_nh = ten_cn_nh;
    }

    public String getTen_cn_nh() {
        return ten_cn_nh;
    }

    public void setHan_muc_no(String han_muc_no) {
        this.han_muc_no = han_muc_no;
    }

    public String getHan_muc_no() {
        return han_muc_no;
    }

    public void setDu_dau_ngay(String du_dau_ngay) {
        this.du_dau_ngay = du_dau_ngay;
    }

    public String getDu_dau_ngay() {
        return du_dau_ngay;
    }

    public void setPs_thu(String ps_thu) {
        this.ps_thu = ps_thu;
    }

    public String getPs_thu() {
        return ps_thu;
    }

    public void setPs_chi(String ps_chi) {
        this.ps_chi = ps_chi;
    }

    public String getPs_chi() {
        return ps_chi;
    }

    public void setMa_nh(String ma_nh) {
        this.ma_nh = ma_nh;
    }

    public String getMa_nh() {
        return ma_nh;
    }

    public void setTen_nh(String ten_nh) {
        this.ten_nh = ten_nh;
    }

    public String getTen_nh() {
        return ten_nh;
    }

    public void setLan_tcuu(String lan_tcuu) {
        this.lan_tcuu = lan_tcuu;
    }

    public String getLan_tcuu() {
        return lan_tcuu;
    }

    public void setNgay_tcuu(String ngay_tcuu) {
        this.ngay_tcuu = ngay_tcuu;
    }

    public String getNgay_tcuu() {
        return ngay_tcuu;
    }

    public void setMa_kb(String ma_kb) {
        this.ma_kb = ma_kb;
    }

    public String getMa_kb() {
        return ma_kb;
    }

    public void setTen_kb(String ten_kb) {
        this.ten_kb = ten_kb;
    }

    public String getTen_kb() {
        return ten_kb;
    }

    public void setKb_id(String kb_id) {
        this.kb_id = kb_id;
    }

    public String getKb_id() {
        return kb_id;
    }

    public void setChenh_lech(String chenh_lech) {
        this.chenh_lech = chenh_lech;
    }

    public String getChenh_lech() {
        return chenh_lech;
    }

    public void setNguoi_tao(Long nguoi_tao) {
        this.nguoi_tao = nguoi_tao;
    }

    public Long getNguoi_tao() {
        return nguoi_tao;
    }

    public void setSo_lan(String so_lan) {
        this.so_lan = so_lan;
    }

    public String getSo_lan() {
        return so_lan;
    }

    public void setTrang_thai(String trang_thai) {
        this.trang_thai = trang_thai;
    }

    public String getTrang_thai() {
        return trang_thai;
    }

    public void setMa_kq(String ma_kq) {
        this.ma_kq = ma_kq;
    }

    public String getMa_kq() {
        return ma_kq;
    }

    public void setMa(String ma) {
        this.ma = ma;
    }

    public String getMa() {
        return ma;
    }

    public void setTen(String ten) {
        this.ten = ten;
    }

    public String getTen() {
        return ten;
    }

    public void setMa_cha(String ma_cha) {
        this.ma_cha = ma_cha;
    }

    public String getMa_cha() {
        return ma_cha;
    }

    public void setId_cha(String id_cha) {
        this.id_cha = id_cha;
    }

    public String getId_cha() {
        return id_cha;
    }

    public void setCap(String cap) {
        this.cap = cap;
    }

    public String getCap() {
        return cap;
    }

    public void setNhkb_nhan_cn(String nhkb_nhan_cn) {
        this.nhkb_nhan_cn = nhkb_nhan_cn;
    }

    public String getNhkb_nhan_cn() {
        return nhkb_nhan_cn;
    }

    public void setMsg_id(String msg_id) {
        this.msg_id = msg_id;
    }

    public String getMsg_id() {
        return msg_id;
    }
public void setNh_id(String nh_id) {
        this.nh_id = nh_id;
    }

    public String getNh_id() {
        return nh_id;
    }

    public void setLoai_tien(String loai_tien) {
        this.loai_tien = loai_tien;
    }

    public String getLoai_tien() {
        return loai_tien;
    }
}
