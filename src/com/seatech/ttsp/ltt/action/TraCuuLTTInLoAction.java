...
import com.seatech.framework.datamanager.MultiReportUtility;
...
public class TraCuuLTTInLoAction extends AppAction {
    ...
    public ActionForward list(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            if (lttForm.getDen_ngay_nhan() != null &&
                ...
                szWhereClause.append("and trunc(t.ngay_nhan) <= to_date(?,'dd/mm/yyyy') ");
                param_ltt = new Parameter(strDen_ngay, true);
                v_Param_ltt.add(param_ltt);
            }
            //loai lenh
            HttpSession session = request.getSession();
        ...
        try {
            ...
            if (reportStream != null) {
                ...
                if (resultSet == null) {
                    ...
                    String strParameter = "";
                    MultiReportUtility rpUtilites = new MultiReportUtility();
    ...
    public ActionForward view(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            if (lstID != null) {
                String regex = ",";
                iter = lstID.split(regex);
                iSize = iter.length;
                List<JasperPrint> jasperPrints = new ArrayList<JasperPrint>();
                for (int i = 0; i < iSize; i++) {
                    if (iter[i] != null && !STRING_EMPTY.equals(iter[i])) {
                        sbSubHTML.append("<input type=\"hidden\" name=\"id\" value=\"" +
                                         lttForm.getId() +
                                         "\" id=\"id\"></input>");
                        String whereClause = " t.id = ? ";
                        vParam = new Vector();
                        vParam.add(new Parameter(iter[i], true));
                        ...
                        if (lttVO != null) {
                            ...
                            if (lttVO.getNhkb_chuyen() != null) {
                                ...
                                paramNH.add(new Parameter(lttVO.getNhkb_chuyen(),
                                                          true));
                                DMNHangVO nhVO =
                                    nhDAO.getDMNH(strWhereClauseNH, paramNH);
                                if (nhVO != null) {
                                    if (nhVO.getMa_nh().substring(2,
                                                                  5).equals("201")) {
                                        strNH = "201";
                                ...
                                if (lttVO.getNt_code().equals("VND")) {
                                    ...
                                    if ("02".equals(lttVO.getLoai_hach_toan())) {
                                        strParamForLTT =
                                                "NGOAI_TE_HACH_TOAN_CHO_XLY_LTT_NO";
                                    } else {
                                        strParamForLTT =
                                                "NGOAI_TE_HACH_TOAN_DUNG_LTT";
                                    }
                                }
                                ...
                                paramsForTSo.add(new Parameter(strParamForLTT,
                                                               true));
                                ...
                                if ("02".equals(lttVO.getLoai_hach_toan())) {
                                    if (lttVO.getNt_code().equals("VND")) {
                                        strParamForLTT =
                                                "HACH_TOAN_CHO_XLY_LTT_CO";
                                    } else {
                                        strParamForLTT =
                                                "NGOAI_TE_HACH_TOAN_CHO_XLY_LTT_CO";
                                    }
                                    paramsForTSo = new Vector();
                                    paramsForTSo.add(new Parameter(strParamForLTT,
                                                                   true));
                                    paramsForTSo.add(new Parameter(strMaNH,
                                                                   true));
                                    tsoVO =
                                            tsoDAO.getDmTSoHToan(strWhereClauseTSo,
                                                                 paramsForTSo);
                                    strTKCo =
                                            tsoVO != null ? tsoVO.getTktn() : "";
                                } else {
                                    if (lttVO.getSo_tk_nhan().length() >= 4)
                                        strTKCo =
                                                lttVO.getSo_tk_nhan().substring(0,
                                                                                4);
                                }
                                parameterMap.put("CO_TK", strTKCo);

                                parameterMap.put("P_ID",
                                                 lttVO.getId().toString());
                                parameterMap.put("Type_LTT", strType_LTT);
                                parameterMap.put("p_imageApprove",
                                                 strPathImage);
                ...
                String strTypePrintAction =
                    ...
                    "" :
                    request.getParameter(AppConstants.REQUEST_ACTION).toString();
                String strActionName = "lttDenRptAction.do";
                String strParameter = "";

                MultiReportUtility rpUtilites = new MultiReportUtility();
                rpUtilites.exportMultiReport(jasperPrints, strTypePrintAction,
                                             response, reportName, strPathFont,
                                             strActionName,
                                             sbSubHTML.toString(),
                                             strParameter);
