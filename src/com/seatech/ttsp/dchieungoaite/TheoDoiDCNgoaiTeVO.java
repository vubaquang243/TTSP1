package com.seatech.ttsp.dchieungoaite;

public class TheoDoiDCNgoaiTeVO {
  private String bkid;
  private String ttsp;
  private String pht;
  private String xnth;
  private String loaiqt;
  private String ma_nh_kb;
  private String ma_nh;
  private String tong_dv;
    private String ma_kb;
    private String ten_ngan_hang;
    private String ten_kb_huyen;
    private String ten_kb_tinh;
    private String sum_dvi;
    private String loai_qtoan_066;
    private String trang_thai_066;
    
    private String id;
    private String lan_dc;
    private String ngay_dc;
    private String send_bank;
    private String receive_bank;
    private String bk_id;
    private String ket_qua;
    private String ket_qua_dxn_thop;
    private String tthai_dxn_thop;
    private String ten;
    private String tthai_066;

      public void setBkid(String bkid) {
          this.bkid = bkid;
      }

      public String getBkid() {
          return bkid;
      }

      public void setTtsp(String ttsp) {
          this.ttsp = ttsp;
      }

      public String getTtsp() {
          return ttsp;
      }

      public void setPht(String pht) {
          this.pht = pht;
      }

      public String getPht() {
          return pht;
      }

      public void setXnth(String xnth) {
          this.xnth = xnth;
      }

      public String getXnth() {
          return xnth;
      }

      public void setLoaiqt(String loaiqt) {
          this.loaiqt = loaiqt;
      }

      public String getLoaiqt() {
          return loaiqt;
      }

      public void setMa_nh_kb(String ma_nh_kb) {
          this.ma_nh_kb = ma_nh_kb;
      }

      public String getMa_nh_kb() {
          return ma_nh_kb;
      }

      public void setMa_nh(String ma_nh) {
          this.ma_nh = ma_nh;
      }

      public String getMa_nh() {
          return ma_nh;
      }

      public void setTong_dv(String tong_dv) {
          this.tong_dv = tong_dv;
      }

      public String getTong_dv() {
          return tong_dv;
      }

      public void setMa_kb(String ma_kb) {
          this.ma_kb = ma_kb;
      }

      public String getMa_kb() {
          return ma_kb;
      }

      public void setTen_ngan_hang(String ten_ngan_hang) {
          this.ten_ngan_hang = ten_ngan_hang;
      }

      public String getTen_ngan_hang() {
          return ten_ngan_hang;
      }

      public void setTen_kb_huyen(String ten_kb_huyen) {
          this.ten_kb_huyen = ten_kb_huyen;
      }

      public String getTen_kb_huyen() {
          return ten_kb_huyen;
      }

      public void setTen_kb_tinh(String ten_kb_tinh) {
          this.ten_kb_tinh = ten_kb_tinh;
      }

      public String getTen_kb_tinh() {
          return ten_kb_tinh;
      }

      public void setSum_dvi(String sum_dvi) {
          this.sum_dvi = sum_dvi;
      }

      public String getSum_dvi() {
          return sum_dvi;
      }

      public void setLoai_qtoan_066(String loai_qtoan_066) {
          this.loai_qtoan_066 = loai_qtoan_066;
      }

      public String getLoai_qtoan_066() {
          return loai_qtoan_066;
      }

      public void setTrang_thai_066(String trang_thai_066) {
          this.trang_thai_066 = trang_thai_066;
      }

      public String getTrang_thai_066() {
          return trang_thai_066;
      }

      public void setId(String id) {
          this.id = id;
      }

      public String getId() {
          return id;
      }

      public void setLan_dc(String lan_dc) {
          this.lan_dc = lan_dc;
      }

      public String getLan_dc() {
          return lan_dc;
      }

      public void setNgay_dc(String ngay_dc) {
          this.ngay_dc = ngay_dc;
      }

      public String getNgay_dc() {
          return ngay_dc;
      }

      public void setSend_bank(String send_bank) {
          this.send_bank = send_bank;
      }

      public String getSend_bank() {
          return send_bank;
      }

      public void setReceive_bank(String receive_bank) {
          this.receive_bank = receive_bank;
      }

      public String getReceive_bank() {
          return receive_bank;
      }

      public void setBk_id(String bk_id) {
          this.bk_id = bk_id;
      }

      public String getBk_id() {
          return bk_id;
      }

      public void setKet_qua(String ket_qua) {
          this.ket_qua = ket_qua;
      }

      public String getKet_qua() {
          return ket_qua;
      }

      public void setKet_qua_dxn_thop(String ket_qua_dxn_thop) {
          this.ket_qua_dxn_thop = ket_qua_dxn_thop;
      }

      public String getKet_qua_dxn_thop() {
          return ket_qua_dxn_thop;
      }

      public void setTthai_dxn_thop(String tthai_dxn_thop) {
          this.tthai_dxn_thop = tthai_dxn_thop;
      }

      public String getTthai_dxn_thop() {
          return tthai_dxn_thop;
      }

      public void setTen(String ten) {
          this.ten = ten;
      }

      public String getTen() {
          return ten;
      }

      public void setTthai_066(String tthai_066) {
          this.tthai_066 = tthai_066;
      }

      public String getTthai_066() {
          return tthai_066;
      }
  }