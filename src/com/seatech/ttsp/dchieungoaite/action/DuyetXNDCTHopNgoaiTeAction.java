...
public class DuyetXNDCTHopNgoaiTeAction extends AppAction {

    public ActionForward executeAction(ActionMapping mapping, ActionForm form,
                                       HttpServletRequest request,
                                       HttpServletResponse response) throws Exception {
        if (!checkPermissionOnFunction(request, "DCHIEU.NT_DUYET_DNQT")) {
...
//          String strCHK=" AND trunc(ngay_qtoan) >= trunc(sp_util_pkg.fnc_get_ngay_lam_viec_truoc (SYSDATE -"+giatri_ts+")) AND nhkb_chuyen='" + kb_chuyen + "'";
          ...
          //          Long qtoan_tdong = dnqtVO.getQtoan_tdong();
            ...
            ArrayList<DuyetKQDCVO> colDChieu = (ArrayList<DuyetKQDCVO>)dao.getListXNDChieu_066(kb_chuyen, tu_ngay, null);
            ...
            String loai_tien = request.getParameter("loai_tien") == null ? DCForm.getLoai_tien() : request.getParameter("loai_tien");//ManhNV them 20150122
            String strKB = "";
            strKB =
                    " AND a.kb_id=" + id_kb + " AND b.ma_nh='" + receive_bank  + "' AND a.ma_nt='"+loai_tien+"' AND a.loai_tk <> '01' ";//ManhNV them 20150122
...
//            String loai_tien = request.getParameter("loai_tien") == null ? DCForm.getLoai_tien() : request.getParameter("loai_tien");
            String strW = "";
            strW = "'" + id + "'";
            String strTTSP = strW;
            String strPHT = " SELECT pht_id from sp_065_ngoai_te where id='" + id + "'";
            // String strTHDC= strW ;
        ...
        try {
            ...
            if (isTokenValid(request)) {
                ...
                if (bVerifySignature) {
                    ...
                    dcVO.setManager(manager);
                    msg_id = sms.sendMessageNgoaiTe(id_066, user_id);
                    dcVO.setId(id);
...
//            e.printStackTrace();
    ...
    public ActionForward updateExc(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            String ngay_ks_xn_qtoan = StringUtil.DateToString(new Date(), AppConstants.DATE_TIME_FORMAT_SEND_ORDER);
            if (strUserInfo.indexOf(AppConstants.NSD_TTV) != -1) {
                request.setAttribute("chucdanh", strUserInfo);
            } else if (strUserInfo.indexOf(AppConstants.NSD_KTT) != -1) {
                request.setAttribute("chucdanh", strUserInfo);
            ...
            if (isTokenValid(request)) {
                ...
                String id = DCForm.getId();
                String id_066 = DCForm.getId_066() == null ? "" : DCForm.getId_066();
                if (id_066 == null || "".equals(id_066)) {
                    ...
                    String str066 =
                        " and id ='"+ id_066 +
                        "' and to_char(ngay_qtoan,'DD/MM/RRRR')='" + ngay_dc +
                        "' AND trang_thai<> '03'";
                    col066 = dao.getData066(str066, null);
                    if (col066.size() >= 1) {
                        if (chkdate.equals(ngay_dc) && "01".equals(loai_qtoan)) {
                            dcVO.setTthai_dxn_thop("00");
                            dcVO.setId(id);
                            dao.DuyetDCTHop(dcVO);
                        }
