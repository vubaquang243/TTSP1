...
public class TKNHKBacDAO extends AppDAO{
  ...
  private static String CLASS_NAME_DAO = "com.seatech.ttsp.tknhkb.TKNHKBacDAO";
 ...
 * */
  ...
  public Collection getNH_KB(String strWhere, Vector vParam) throws Exception{
      try{
        String strSQL = "SELECT DISTINCT b.ma_nh, b.ten" + 
              " FROM sp_tknh_kb a, sp_dm_ngan_hang b, sp_dm_htkb c where a.nh_id=b.id and a.kb_id = c.id ";
        if (strWhere != null) {
            strSQL += strWhere;
        }
  ...
  public Collection getLT_NH_KB(String strWhere, Vector vParam) throws Exception{
      try{
        String strSQL = "SELECT DISTINCT a.ma_nt" + 
              " FROM sp_tknh_kb a, sp_dm_ngan_hang b, sp_dm_htkb c where a.nh_id=b.id and a.kb_id = c.id ";
        if (strWhere != null) {
            strSQL += strWhere;
        }
  ...
  public Collection getLstTK(String strWhere,
      ...
      try {
          String strSQL =
              ...
              " WHERE	b.shkb = c.ma AND a.kb_id = c.id AND a.nh_id = d.id" + strWhere + " order by b.shkb";
  ...
  public boolean checkExist(String ma_nh, String so_tk) throws Exception {
      ...
      try {
          String strSQL =
              "SELECT a.id "+
              " FROM sp_tknh_kb a where a.nh_id='"+ ma_nh + "'  and a.so_tk='" + so_tk+"'";
  ...
  public int deletetTKNHKB(TKNHKBacVO vo) throws Exception {
    ...
    try {
        sqlbuff = new StringBuffer(" DELETE  sp_tknh_kb  a  WHERE a.id= ? ");
  ...
  public TKNHKBacVO getTK_NH_KB_VO_2(String strWhere,
    ...
    public Collection getNganHangCoTaiKhoan(String condition, Vector params) throws Exception {
        String sql = "select distinct ten , ma_nh " + 
              "from sp_dm_ngan_hang a " + 
              "join sp_tknh_kb b " + 
              "on a.id = b.nh_id " + 
              "where 1=1 ";
